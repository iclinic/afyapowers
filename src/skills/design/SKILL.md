...
This step runs ONLY when Figma discovery produced a Node Map — skip it entirely for backend/API/CLI/lib features and for UI work with no Figma. Immediately after `{{skill:reading-figma-designs}}` returns (the Node Map and `## Recursos do Figma` must already exist) and BEFORE the clarifying questions, run this lean check directly (no sub-skill dispatch):
