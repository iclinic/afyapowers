...
3. **Analyze the design system (only when Figma is present)** — right after `reading-figma-designs` returns the Node Map, invoke `{{skill:analyzing-design-system}}` to build the `## Árvore de Componentes de DS`; confirm proposed derivative names and ambiguous verdicts with the user in this phase (R7/R14). Skip entirely when there is no Figma (see below)
...
This step runs ONLY when Figma discovery produced a Node Map — skip it entirely for backend/API/CLI/lib features and for UI work with no Figma. Immediately after `{{skill:reading-figma-designs}}` returns (the Node Map and `## Recursos do Figma` must already exist — the DS tree can only be built once they do) and BEFORE the clarifying questions, invoke `{{skill:analyzing-design-system}}`.
