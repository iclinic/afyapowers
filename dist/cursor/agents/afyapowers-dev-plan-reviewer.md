...
| Figma Acceptance Measures | If the spec has a `## Contrato de Layout`: **every** UI task's `**Figma:**` block carries breakpoints and **Medidas de aceite**. `UI Screen` tasks route to the screen implementer, whose fidelity verification is mandatory and reads those measures from the `**Figma:**` block; without them the verifier fails pre-flight and the task ends in a guaranteed BLOCKING. Not required when there is no `## Contrato de Layout` |
