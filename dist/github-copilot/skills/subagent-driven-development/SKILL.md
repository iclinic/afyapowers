...
- **DONE_WITH_CONCERNS**: read concerns and classify each by **what it describes**, not by the tag the implementer gave it (**BLOCKING** vs non-blocking **CONCERN**). Mark the task `completed` and commit its work (Step 6.5) so it isn't lost — but track BLOCKING and non-blocking concerns in **separate** lists. If a concern indicates the task is fundamentally broken, treat as `BLOCKED` instead.
  ...
  - **A concern describing a visible divergence from the Figma screens is BLOCKING, whatever its tag.** Wrong element state, wrong glyph, an interaction that differs, or a configuration a screen renders that the component cannot express — reclassify it as BLOCKING even when the implementer filed it as a routine concern justified by "decisão do usuário" or "escopo reduzido": a recorded decision covers a visible consequence only if the decision in `design.md` names that consequence.
...
**DONE_WITH_CONCERNS:** Read concerns and classify each by what it describes, not by the implementer's tag (**BLOCKING** vs non-blocking **CONCERN**). Store them in separate lists and mark `completed`. If the concern indicates the task is fundamentally broken (e.g., "I couldn't get tests to pass", "Core dependency is missing and I had to stub the entire integration"), treat as `BLOCKED` instead. BLOCKING concerns (e.g. a component substitution that doesn't match Figma, an unconfirmed host-height assumption, behavior that differs from the design) are collected separately and gate the phase via the `implementing` skill. Examples of non-blocking concerns: "I'm not sure this edge case is handled correctly", "The API response format might differ in production", "This works but the approach feels fragile." A concern that **defers verification** to a later phase instead of performing it (e.g. "vou validar isso na review", "will check this later") is always **BLOCKING**, never non-blocking — even if the implementer filed it as a routine ressalva — because it is a verification requirement that only closes with actual evidence, not a promise to verify eventually. Likewise, a concern describing a **visible divergence from the Figma screens** (wrong state, wrong glyph, a rendered configuration the component cannot express) is always **BLOCKING**, even when justified as "decisão do usuário"/"escopo reduzido" — unless the decision recorded in `design.md` names that exact consequence.
