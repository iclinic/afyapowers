...
- **Compose from:** [COMPOSE_FROM] — for a **composite** component (built from sub-components, e.g. `multi-select = input + menu`), the list of child components this node composes, each as `{ code name, import path }`. They **already exist in code** by the time you run (dispatched earlier, leaves→root). You MUST **import and compose** them — never reimplement them. Empty when the component composes nothing. (Distinct from `[BASE_COMPONENT]`: that is a single base you *derive from*; `[COMPOSE_FROM]` is N peer children you *compose*.)
...
The detailed rules for each mode — the reuse-vs-derive cut, the wrapper pattern, additive-update constraints, and combinatorial-props guidance — live in `references/ds-implementation.md` in the `afyapowers:figma-component` skill. Read it and follow it; the summaries below only orient you to the right mode. Do not duplicate those rules here.

- **`implementar`** (default when verdict is absent) — Build the **generic** component from scratch with ALL its variants. Each Figma variant axis (`size`, `type`, `state`, …) becomes an **independent prop**, never a cartesian-product union (see `ds-implementation.md` §3.3). This is the full original workflow: proceed through Steps 1–7 exactly as written below. **If `[COMPOSE_FROM]` is non-empty, this is a composite** (e.g. `multi-select = input + menu`): build the new component's structure per Figma, but for **each listed child, import and compose the existing component** — do NOT reimplement it (see Implementation Rule 8 and `ds-implementation.md` §2.3). Steps 1–7 then apply to the composite's own surface (its layout, the wiring/props between children, and any extra elements), not to re-building the children.
...
- **`implementar`** — Same as check C: every variant axis is an independent prop, all variants covered. **If `[COMPOSE_FROM]` is non-empty (composite):** confirm each listed child is **imported and composed** (its import resolves) rather than reimplemented/inlined; any reinlined or reimplemented child is an ISSUE (Implementation Rule 8). A listed child whose import does not resolve or that does not exist is a BLOCKING concern.
...
8. **Compose listed children — never reimplement them.** Any Figma node inside this component that matches an entry in `[COMPOSE_FROM]` (or the `[BASE_COMPONENT]` for `derivar`) **already exists in code** — the orchestrator implemented/imported it first (leaves→root order). **Import it and compose it**; never inline, copy, or reimplement its source. This mirrors the `figma-design-implementer`'s import-existing rule, scoped to a single composite/derived component. Use each entry's resolved import path. If a listed child's import does not resolve, or the required child does not actually exist, report a **BLOCKING** concern (do not reimplement it to paper over the gap). Follow the composition rules in `ds-implementation.md` §2 (derivar, single base) / §2.3 (composite, N peers).
...
- **Verdict mode** — which of `implementar` / `importar` / `atualizar` / `derivar` you ran, and the mode-specific outcome:
  - `implementar` — if `[COMPOSE_FROM]` was non-empty (composite), list each child you **imported and composed** and its resolved import path; confirm none were reimplemented.
