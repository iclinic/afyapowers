...
| Figma Acceptance Measures | If the spec has a `## Contrato de Layout`: every Layer 1/Layer 2 Figma task's `**Figma:**` block carries breakpoints and **Medidas de aceite** (from the spec's `## Contrato de Layout`). Not required when there is no `## Contrato de Layout` |
...
**When the spec has a `## Contrato de Layout`, also check:**
...
- A Layer 1/Layer 2 Figma task's `**Figma:**` block missing **Medidas de aceite**
...
**When there is no `## Contrato de Layout`:** none of the checks above apply — do not require a skeleton task, acceptance measures, or breakpoint reconciliation.
